...
class AgentsUtils:
    ...
    async def print_stream_events(result: AsyncIterator[StreamEvent]) -> None:
        ...
        # 在函数内部导入OrchestraStreamEvent以避免循环导入
        from utu.agents.orchestra.common import OrchestraStreamEvent
            ...
            if isinstance(event, RawResponsesStreamEvent):
                ...
                if item.type == "message_output_item":
                    ...
                    PrintUtils.print_bot(
                        f"<{item.agent.name}> [工具调用] {item.raw_item.name}({item.raw_item.arguments})"
                    )
                elif item.type == "tool_call_output_item":
                    PrintUtils.print_tool(f"<{item.agent.name}> [工具输出] {item.output}")  # item.raw_item
                ...
                PrintUtils.print_info(f">> 新智能体: {event.new_agent.name}")
            elif isinstance(event, OrchestraStreamEvent):
                # 处理OrchestraStreamEvent事件，显示更多详细信息
                if event.name == "plan":
                    PrintUtils.print_info(">> 计划阶段:")
                    PrintUtils.print_info(f"   分析: {event.item.analysis}")
                    PrintUtils.print_info("   任务列表:")
                    for i, subtask in enumerate(event.item.todo, 1):
                        PrintUtils.print_info(f"     {i}. {subtask.task} (负责智能体: {subtask.agent_name})")
                elif event.name == "worker":
                    PrintUtils.print_info(f">> 工作阶段 - 任务: {event.item.task}")
                    PrintUtils.print_info(f"   输出: {event.item.output}")
                elif event.name == "report":
                    PrintUtils.print_info(">> 报告阶段:")
                    PrintUtils.print_info(f"   最终报告: {event.item.output}")
            else:
