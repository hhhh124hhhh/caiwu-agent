...
{content}
    ...
    try:
        ...
        if report_agent_config:
            ...
            # 生成财务指标图表 - 使用正确的数据格式
            if tabular_toolkit:
                charts_response = tabular_toolkit.generate_charts(
                    data_json='{"title": "陕西建工盈利能力指标", "x_axis": ["毛利率", "净利率", "ROE", "ROA"], "series": [{"name": "数值", "data": [12.0, 1.92, 2.82, 0.32]}]}',
                    chart_type='bar',
                    output_dir=str(workspace_path)
                )
                print(f"📊 图表生成: {charts_response}")
