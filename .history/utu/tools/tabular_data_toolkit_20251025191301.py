        ...
        try:
            ...
            if chart_type == 'line':
                ...
                for i, item in enumerate(series):
                    ...
                    if isinstance(pie_data, list) and pie_data and isinstance(pie_data[0], dict):
                        # 处理{name, value}格式
                        labels = [d.get('name', f'项目{j+1}') for j, d in enumerate(pie_data)]
                        values = [d.get('value', 0) for d in pie_data]
                    else:
                        # 处理简单数值列表格式
                        labels = x_axis if len(x_axis) == len(pie_data) else [f'项目{j+1}' for j in range(len(pie_data))]
                        values = pie_data
                    
                    # 绘制饼图
                    plt.pie(values, labels=labels, autopct='%1.1f%%', startangle=90, 
                           colors=colors[:len(labels)], wedgeprops={'edgecolor': 'w', 'linewidth': 1})
                    plt.axis('equal')  # 保证饼图是圆形的
                    plt.title(f"{title} - {series_name}", fontsize=16, fontweight='bold')
                    
                    # 添加图例
                    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=10)
                    
                    # 保存图表
                    pie_filename = os.path.join(output_dir, f"{title.replace(' ', '_')}_{series_name}_饼图.png")
                    plt.tight_layout()
